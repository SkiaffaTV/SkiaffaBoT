...
class SolanaClient:
    ...
    async def confirm_transaction(
        ...
        """Wait for transaction confirmation and verify its status.
        ...
        Returns:
            Whether transaction was confirmed successfully
        """
        client = await self.get_client()
        try:
                ...
                if response.value and response.value[0] is not None:
                    ...
                    if status.confirmation_status == Confirmed:
                        if status.err is None:
                            logger.debug(f"Transaction {signature} confirmed on chain")
                            return True
                        else:
                            logger.error(f"Transaction {signature} failed: {status.err}")
                            return False
                    logger.debug(f"Transaction {signature} not yet confirmed, retrying...")
                await asyncio.sleep(2)
            logger.error(f"Transaction {signature} not found after 5 attempts")
            return False
        except Exception as e:
    ...
    async def get_transaction(self, signature: str) -> Optional[dict[str, Any]]:
        ...
        client = await self.get_client()
        try:
            response = await client.get_transaction(signature, encoding="jsonParsed")
            return response.value
        except Exception as e:
            logger.error(f"Failed to get transaction {signature}: {e!s}")
            return None
