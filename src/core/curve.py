...
# Initial real token reserves for bonding curve (based on pump.fun specification)
INITIAL_REAL_TOKEN_RESERVES: Final[int] = 793100000000000
...
class BondingCurveState:
    ...
    def get_progress(self) -> float:
        ...
        Returns:
            Progress percentage (0 to 100). Returns 0 if real_token_reserves is invalid or exceeds initial reserves.

        Notes:
            Progress is calculated as: 1 - (real_token_reserves / INITIAL_REAL_TOKEN_RESERVES)
        """
        if self.real_token_reserves <= 0 or self.real_token_reserves >= INITIAL_REAL_TOKEN_RESERVES:
            logger.warning(
                f"Invalid real_token_reserves: {self.real_token_reserves}. Returning 0% progress."
            )
            return 0.0
        progress = 1 - (self.real_token_reserves / INITIAL_REAL_TOKEN_RESERVES)
        return progress * 100
